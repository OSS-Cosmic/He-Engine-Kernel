...
{
	size_t allocSize = HE_ALIGN_TO( reqSize, 4 ); // we round up to multiples of uint32_t
...
void RI_ResourceEndCopyBuffer( struct RIDevice_s *device, struct RIResourceUploader_s *res, struct RIResourceBufferTransaction_s *trans ) {
#if ( DEVICE_IMPL_VULKAN )
	{
		if( !__ResourceInTransitionBuffer(device, res, trans->target)) {
			...
			dependencyInfo.bufferMemoryBarrierCount = HE_ARRAY_COUNT( bufferBarriers );
...
void RI_InsertTransitionBarriers( struct RIDevice_s *device, struct RIResourceUploader_s *res, struct RICmd_s *cmd )
{
#if ( DEVICE_IMPL_VULKAN )
	{
		...
		while( true ) {
			while( postBufferIdx < arrlen( res->postBufferBarriers ) && numBufferBarriers < HE_ARRAY_COUNT( bufferBarriers ) ) {
			...
			while( postImageIdx < arrlen( res->postImageBarriers ) && numImageBarriers < HE_ARRAY_COUNT( imageBarriers ) ) {
...
{
	const uint64_t alignedRowPitch = HE_ALIGN_TO( trans->rowPitch, device->physicalAdapter.uploadBufferTextureRowAlignment );
	const uint64_t alignedSlicePitch = HE_ALIGN_TO( trans->sliceNum * alignedRowPitch, device->physicalAdapter.uploadBufferTextureSliceAlignment );
...
//	range.baseMipLevel = HE_MIN(a1.baseMipLevel, a2.baseMipLevel);
//	range.baseArrayLayer = HE_MIN(a1.baseArrayLayer, a2.baseArrayLayer);
//	range.layerCount = HE_MAX(a1.baseArrayLayer + a1.layerCount, a2.baseArrayLayer + a2.layerCount) - range.baseArrayLayer;
//	range.layerCount = HE_MAX(a1.baseMipLevel + a1.levelCount , a2.baseMipLevel + a2.levelCount) - range.baseMipLevel;
//	return range;
//} 
...
void RI_ResourceEndCopyTexture( struct RIDevice_s *device, struct RIResourceUploader_s *res, struct RIResourceTextureTransaction_s *trans )
{
...
#if ( DEVICE_IMPL_VULKAN )
	{
		...
		if( !foundInTransition ) {
			...
			VkDependencyInfo dependencyInfo = { VK_STRUCTURE_TYPE_DEPENDENCY_INFO };
			dependencyInfo.imageMemoryBarrierCount = HE_ARRAY_COUNT( imageBarriers );
			dependencyInfo.pImageMemoryBarriers = imageBarriers;
			vkCmdPipelineBarrier2( res->vk.cmdSets[res->syncIndex % RI_RESOURCE_NUM_COMMAND_SETS].cmd, &dependencyInfo );
		...
		}
		{
			...
			//assert( res->syncIndex % RI_RESOURCE_NUM_COMMAND_SETS < HE_ARRAY_COUNT( res->vk.cmdSets ) );
