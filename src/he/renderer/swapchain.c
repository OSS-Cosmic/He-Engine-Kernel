...
#include "he/core/ri.h"
#include "he/core/ri_format.h"
...
int InitRISwapchain( struct RIDevice_s *dev, struct RISwapchainDesc_s *init, struct RISwapchain_s *swapchain )
{
	...
	assert( init );
	assert( swapchain );
	assert( init->imageCount <= HE_ARRAY_COUNT( swapchain->vk.images ) && init->imageCount > 0 );
  ...
  for( size_t j = 0; j < HE_ARRAY_COUNT( preferredModeList ); j++ ) {
