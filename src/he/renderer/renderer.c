...
#if ( DEVICE_IMPL_VULKAN )
	{
		...
		if( adapters ) {
			...
			for( size_t i = 0; i < deviceGroupNum; i++ ) {
				...
				// selected preset
				for(size_t i = 0; i < HE_ARRAY_COUNT(gpuPCPresets); i++) {
				...
				physicalAdapter->attachmentMaxDim = HE_MIN( limits->maxFramebufferWidth, limits->maxFramebufferHeight );
				...
				physicalAdapter->bufferShaderResourceOffsetAlignment = (uint32_t)HE_MAX( limits->minTexelBufferOffsetAlignment, limits->minStorageBufferOffsetAlignment );
...
int InitRIDevice( struct RIRenderer_s *renderer, struct RIDeviceDesc_s *init, struct RIDevice_s *device )
{
...
#if ( DEVICE_IMPL_VULKAN )
	{
		...
		for( uint32_t configureIdx = 0; configureIdx < HE_ARRAY_COUNT( configureQueue ); configureIdx++ ) {
			//bool found = false;
			...
			if( createInfo->queueCount >= queueFamilyProps[createInfo->queueFamilyIndex].queueCount) {
				...
				for( size_t i = 0; i < HE_ARRAY_COUNT( device->queues ); i++ ) {
		...
		//for( uint32_t initIdx = 0; initIdx < HE_ARRAY_COUNT( configureQueue ); initIdx++ ) {
		...
		//		for( ; createQueueIdx < HE_ARRAY_COUNT( deviceQueueCreateInfo ); createQueueIdx++ ) {
		...
		//				if(isQueueEmpty) {
		//					deviceCreateInfo.queueCreateInfoCount = HE_MAX( deviceCreateInfo.queueCreateInfoCount, createQueueIdx + 1);
		...
		//		assert( createQueueIdx < HE_ARRAY_COUNT( deviceQueueCreateInfo ) );
   ...
   // }
		...
		// the request size
		for( size_t q = 0; q < HE_ARRAY_COUNT( device->queues ); q++ ) {
			// the queue
...
int InitRIRenderer( const struct RIBackendInit_s *init, struct RIRenderer_s *renderer )
{
    ...
    renderer->vk.apiVersion = appInfo.apiVersion;
		...
		validationFeatures.enabledValidationFeatureCount = HE_ARRAY_COUNT( enabledValidationFeatures );
		...
		{
			assert( 1 <= HE_ARRAY_COUNT( enabledLayerNames ) );
			...
			for( size_t i = 0; i < enumInstanceLayers; i++ ) {
				...
				if( useLayer ) {
					assert( instanceCreateInfo.enabledLayerCount < HE_ARRAY_COUNT( enabledLayerNames ) );
		...
		}
		{
			...
			for( size_t i = 0; i < extensionNum; i++ ) {
				...
				if( useExtension ) {
					assert( instanceCreateInfo.enabledExtensionCount < HE_ARRAY_COUNT( enabledExtensionNames ) );
