...
typedef void (*HeLogCallbackFn)(void* user_data, const char* message);
typedef void (*HeLogCloseFn)(void* user_data);
typedef void (*HeLogFlushFn)(void* user_data);
...
extern "C"
{
    ...
    void heInitLog(const char* appName, enum HeLogLevel_e level);
    void heExitLog(void);

    void heAddLogFile(const char* filename, enum HeFileMode_e file_mode, enum HeLogLevel_e log_level);
    void heAddLogCallback(const char* id, uint32_t log_level, void* user_data, HeLogCallbackFn callback, HeLogCloseFn close,
                                  HeLogFlushFn flush);

    void heWriteLogVaList(uint32_t level, const char* filename, int line_number, const char* message, va_list args);
    //+V576, function:writeLog, format_arg:4, ellipsis_arg:5
    void heWriteLog(uint32_t level, const char* filename, int line_number, const char* message, ...);
    //+V576, function:writeRawLog, format_arg:3, ellipsis_arg:4
    void heWriteRawLog(uint32_t level, bool error, const char* message, ...);

    //+V576, function:_FailedAssert, format_arg:4, ellipsis_arg:5
    void _he_FailedAssert(const char* file, int line, const char* statement, const char* msg, ...);
