...
#include "he/core/math/math.h"
#include "he/core/hash.h"
#include "he/core/types.h"

#include "he/renderer/defines.h"
#include "he/config.h"
...
struct RIDescriptor {	
	hash_t cookie; // unique id to mark the descriptor 
	uint8_t flags;
...
struct HEGPUDevice {
	struct HEGPUPhysicalAdapter physicalAdapter;
  struct RIRenderer* renderer;
  struct Queue queues[RI_QUEUE_LEN];
  union {
    #if(DEVICE_IMPL_VULKAN)
    struct {
...
static inline bool IsRICmdValid( struct RIRenderer *renderer, struct RICmd *cmd )
{
#if ( DEVICE_IMPL_VULKAN )
...
static inline bool IsRIBufferValid( struct RIRenderer *renderer, const struct RIBuffer *handle )
{
#if ( DEVICE_IMPL_VULKAN )
...
static inline bool IsRITextureValid( struct RIRenderer *renderer, const struct RITexture *handle )
{
#if ( DEVICE_IMPL_VULKAN )
...
static inline uint32_t RIGetQueueFlags(struct RIRenderer* renderer,const struct Queue* queue) {
#if ( DEVICE_IMPL_VULKAN )
...
struct HEDeviceDesc_s {
	struct HEGPUPhysicalAdapter *physicalAdapter;
};
int InitRIRenderer( const struct HEBackendInit *init, struct RIRenderer *renderer );
void ShutdownRIRenderer( struct RIRenderer *renderer );

int EnumerateRIAdapters( struct RIRenderer *renderer, struct HEGPUPhysicalAdapter *adapters, uint32_t *numAdapters );
int InitRIDevice( struct RIRenderer *renderer, struct HEDeviceDesc_s *init, struct HEGPUDevice *device );

void WaitRIQueueIdle( struct HEGPUDevice *device, struct Queue *queue );

int FreeRIDevice( struct HEGPUDevice *dev );
void FreeRIFree( struct HEGPUDevice *dev, struct FreeReq *mem );

// RIDescriptor
void UpdateRIDescriptor( struct HEGPUDevice *dev, struct RIDescriptor *desc ); // after configure an RIDescriptor call update to configure it
void FreeRIDescriptor( struct HEGPUDevice *dev, struct RIDescriptor *desc );
static inline bool IsEmptyDescriptor( struct RIDescriptor *desc ) { return desc->cookie == 0; }

// RITexture
void FreeRITexture( struct HEGPUDevice *dev, struct RITexture *tex );

// RICmd
void FreeRICmd( struct HEGPUDevice *dev, struct RICmd *cmd);

#if DEVICE_IMPL_VULKAN
void VK_ConfigureBufferQueueFamilies( VkBufferCreateInfo *info, struct Queue *queues, size_t numQueues, uint32_t *queueFamiliesIdx, size_t reservedLen );
void VK_ConfigureImageQueueFamilies( VkImageCreateInfo *info, struct Queue *queues, size_t numQueues, uint32_t *queueFamiliesIdx, size_t reservedLen );
void vk_fillQueueFamilies( struct HEGPUDevice *dev, uint32_t *queueFamilies, uint32_t *queueFamiliesIdx, size_t reservedLen );
#endif
