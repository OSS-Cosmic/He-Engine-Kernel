...
static inline VkIndexType ri_vk_RIIndexTypeToVK(enum RIIndexType type) {
	switch(type) {
...
static inline VkBlendFactor ri_vk_RIColorWriteMaskToVK(enum RIColorWriteMask mask) {
	VkBlendFactor ret = 0;
...
static inline VkBlendFactor ri_vk_RIBlendFactorToVK(enum BlendFactor factor) {
	switch (factor) {
		case RI_BLEND_ZERO:
